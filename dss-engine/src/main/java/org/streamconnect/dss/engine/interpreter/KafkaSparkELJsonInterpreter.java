...
/**
 * Kafka-Spark-Elassandra-Json data pipeline Interpreter for Python is created
 * here
 */

@Service
public class KafkaSparkELJsonInterpreter extends StreamFlowInterpreter {

    /** The logger. */
    private static final DSSLogger LOGGER = DSSLogger
            .getLogger(KafkaSparkELJsonInterpreter.class);

    /**
     * default constructor.
     */
    public KafkaSparkELJsonInterpreter() {
     ...
     *            the features
     */
    public KafkaSparkELJsonInterpreter(final String features) {
     ...
     *            the obj pipeline
     */
    public KafkaSparkELJsonInterpreter(final PipelineConfigDto objPipeline) {
        pipelineConf = objPipeline;
    }

    /**
     * function to set pre-requisites for data pipeline.
     *
    ...
    public boolean getPrerequisite(final PipelineConfigDto objPipelineConf) {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : getPrerequisite function : "
        ...
        try {
            ...
            LOGGER.error(
                    ErrorMessageEnum.ENGINE_LAYER_EXCEPTION.getCode().toString()
                            ...
                            .getMessage()
                            + "  "
                            + "KafkaSparkELJsonInterpreter: in buildProcess function : "
                            + "",
                    e);
    ...
    /**
     * create/build data pipeline source related configurations & settings.
     *
     * @return true, if successful
     */
    public boolean buildSource() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : buildSource function : "
                + Constants.START_STATUS);
        boolean bRet = false;
        try {
            ...
            pipeline.setInterpreterProcess("$NUM_THREAD$", Integer.toString(
                    objSrcConfig.getNumThread())/*
                                                 * objSrcConfig.getNumThread ()
                                                 */);
    ...
    public boolean buildEngine() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : buildEngine function : "
                + Constants.START_STATUS);
        boolean bRet = false;
        try {
            ObjectMapper mapper = new ObjectMapper();
    ...
    public boolean buildProcess() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : buildProcess function : "
                + Constants.START_STATUS);
        boolean bRet = false;
        try {
            ...
            // Process - Spark for KafkaSparkELJsonInterpreter ");
        } catch (Exception e) {
            LOGGER.error(
                    ErrorMessageEnum.ENGINE_LAYER_EXCEPTION.getCode().toString()
                            + " : "
    ...
    public boolean genProcessLogic() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : genProcessLogic function : "
                + Constants.START_STATUS);
        boolean bRet = false;
        try {
    ...
    public boolean genParsingLogic() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : genParsingLogic function : "
                + Constants.START_STATUS);
        boolean bRet = false;
        try {
            String strCode = "";
            ...
            if (pipelineConf.getStrDataSchema().getStrSchemaType()
                ...
                strCode += "parsedStream = kafkaStream.map(lambda (key,value)"
                        + ": json.loads(value))\n";
            }
    ...
    public boolean genIterativeLogic() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : genIterativeLogic function : "
                + Constants.START_STATUS);
        boolean bRet = false;
        try {
            String strCode = "";
    ...
    /**
     * create/build data pipeline sink part.
     *
     * @return true, if successful
     */
    public boolean buildSink() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : buildSink function : "
                + Constants.START_STATUS);
        boolean bRet = false;
        try {
            ObjectMapper mapper = new ObjectMapper();
    ...
    /**
     * to get data pipeline executable details.
     *
     * @return the data pipeline
     */
    public PipelineDetails getDataPipeline() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : getDataPipeline function : "
                + Constants.START_STATUS);
    ...
    private String createTableQry(final String strDBName, final String strPersistTo,
        ...
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : createTableQry function : "
                + Constants.START_STATUS);
        String strRet = "";
        try {
    ...
    private String createIndexQry(final String strDBName, final String strPersistTo,
        ...
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : createIndexQry function : "
                + Constants.START_STATUS);
        String strRet = "";
        try {
    ...
    public boolean genSinkTableLogic() {
        LOGGER.info(LogMessageEnum.ENGINE_LAYER_INFO.getMessage() + " "
                + "KafkaSparkELJsonInterpreter : genSinkTableLogic function : "
                + Constants.START_STATUS);
        boolean bRet = false;
        try {
    ...
    private boolean buildUserFunctionLogic() {
        ...
        try {
            ...
            for(UserFunctionDto userFunctionDto : userFunctionDtos){
                ...
                strCodeBase += userFunctionDto
                        .getObjCodeBase().toString()+"\n";
